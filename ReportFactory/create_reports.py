...
def create_worddoc(var_dict, baseline_dict, df_intersects2, df_EntireSet, df_perward, df_MapServiceIntersections,
    ...
    global SpatialFeatureChoice_Text, SpatialFeatureChoice_Text_Single
    fig = {}
    tbl ={}
    heading_no = 1
    fig_nr = 1
    ...
    def build_bar_plots_2(number_of_plots, fig_nr, df_subs):
        ...
        for plot_no in range(1, number_of_plots + 1):
            ...
            if number_of_plots > 1:
                temp_paragraphs[
                    f'{plot_no} cp'] = f"Figure {heading_no}.{temp_fig[plot_no]}: Capital Demand per" \
                                       f" {SpatialFeatureChoice_Text_Single} - " \
    ...
    org_choice = var_dict['org_choice']
    ...
    org_capitalized = org_choice.capitalize()
    # Do our best tp get the wording in the report not to appear clumsy or generated by generating variables for
    # SpatialFeatureChoice_Text and SpatialFeatureChoice_Text_Single
    SpatialFeatureChoice_Text = SpatialFeatureChoice.replace("City of ", "").replace("Metropolitan ", "").replace(
        "Municipality ", "").replace(f'{org_capitalized} ', "").replace("District ", "").replace("Local ", "")

    if SpatialFeatureChoice_Text[0] == "_":
        SpatialFeatureChoice_Text = SpatialFeatureChoice_Text[1:]
    if SpatialFeatureChoice_Text[-1] == "_":
        SpatialFeatureChoice_Text = SpatialFeatureChoice_Text[:-1]
    ...
    list_of_words = re.findall(r'[A-Za-z-]+|\d+', SpatialFeatureChoice_Text_Single)
    for word in list_of_words:
        if word[-1].lower() == 's':
            SpatialFeatureChoice_Text_Single = word[:-1]
            if (SpatialFeatureChoice_Text_Single[-1] == 'e' and SpatialFeatureChoice_Text_Single[-2] == 'i'):
                SpatialFeatureChoice_Text_Single = SpatialFeatureChoice_Text_Single.replace("ie", "y")
        elif word.lower() == 'segment':
            SpatialFeatureChoice_Text_Single = 'Segment'
        elif word.lower() == 'region':
            SpatialFeatureChoice_Text_Single = 'Region'
        elif word.lower() == 'ward':
            SpatialFeatureChoice_Text_Single = 'Ward'
        elif word.lower() == 'area':
            SpatialFeatureChoice_Text_Single = 'Area'
        elif word.lower() == 'development':
            SpatialFeatureChoice_Text_Single = 'Development'
        elif word.lower() == 'settlement':
            SpatialFeatureChoice_Text_Single = 'Settlement'
        elif word.lower() == 'node':
            SpatialFeatureChoice_Text_Single = 'Node'
        elif word.lower() == 'zone':
            SpatialFeatureChoice_Text_Single = 'Zone'
        elif word.lower() == 'poor':
            SpatialFeatureChoice_Text_Single = 'Pro Poor'
        elif word.lower() == 'framework':
            SpatialFeatureChoice_Text_Single = 'Framework'
        elif word.lower() == 'boundary':
            SpatialFeatureChoice_Text_Single = 'Boundary'
        elif word.lower() == 'sdf':
            SpatialFeatureChoice_Text_Single = 'SDF'
        elif word.lower() == 'areascombined':
            SpatialFeatureChoice_Text_Single = 'Area Combined'
    # Set up other vars
    ...
    def create_content(heading_no, fig_nr, tbl_nr):
        ...
        if heading_no == 0:  # General Background Info
            ...
            for feature in Layer_List:
                feature_text_to_use = feature
                if feature_text_to_use[0] == "_":
                    feature_text_to_use = feature_text_to_use[1:]
                if feature_text_to_use[-1] == "_":
                    feature_text_to_use = feature_text_to_use[:-1]
                paragraphs2[f"{teller} xx"] = feature_text_to_use
                teller += 1
            ...
            paragraphs4 = {"1 n": f"The baseline that was queried for this report contains ",
                           ...
                           "5 n": f" projects do not have any recorded intersection (overlap) with the "
                                  f"'{SpatialFeatureChoice_Text}' spatial feature that was selected for this report."
            ...
            paragraphs4[f"{teller} al"] = f"\nIt is important to take note of the " \
                                          ...
                                          f"with the '{SpatialFeatureChoice_Text}' spatial feature when appraising this" \
                                          f" report."
            ...
            paragraphs5a = {"1 n": f"Table {heading_no}.{tbl_nr} below provides a concise summary of key statistical"
                                   ...
                                   f"of the {SpatialFeatureChoice_Text_Single.lower()}s.",
            ...
            paragraphs5b = {"1 n": f"\nTable {heading_no}.{tbl_nr} below provides a concise summary of key statistical"
                                   ...
                                   f"{SpatialFeatureChoice_Text_Single.lower()}.",
                            "2 cp": f"Table {heading_no}.{tbl_nr}: {SpatialFeatureChoice_Text} Analysis - Capital "
                                    f"Demand (R)"
                            }
            paragraphs5c = {"1 n": f"\nThe number of projects per {SpatialFeatureChoice_Text_Single.lower()} and the capital "
                                   f"demand per {SpatialFeatureChoice_Text_Single.lower()} needs to be looked at together for"
                            ...
                            "2 cp": f"Figure {heading_no}.{fig_nr}: The single {SpatialFeatureChoice_Text_Single.lower()} with"
                                    f" the highest value (Nr of Projects & Capital Demand) vs the rest"
                            }
            paragraphs6 = {"1 n": f"The highest number of projects in a single {SpatialFeatureChoice_Text_Single.lower()} is"
                                  f" in ",
                           ...
                           "4 n": f"The highest capital demand in a single {SpatialFeatureChoice_Text_Single.lower()} is in ",
                           "5 b": f"{maximum_cost_feature}. ",
                           "6 i": f"That is {maximum_cost} and this amounts to {max_cost_perc_of_total} of the total"
                                  f" capital demand."}
            paragraphs5d = {"1 cp": f"Figure {heading_no}.{fig_nr+1}: The 5 {SpatialFeatureChoice_Text_Single.lower()}s with the highest"
                                    f" values (Nr of Projects & Capital Demand) vs the rest"
                            }
            paragraphs7 = {"1 n": f"The 5 {SpatialFeatureChoice_Text_Single.lower()}s that collectively have with the highest number"
                           ...
                           "4 n": f"The 5 {SpatialFeatureChoice_Text_Single.lower()}s that collectively have the highest capital"
                                  f" demand are in ",
            ...
            paragraphs8 = {
                "1 n": f"The following figures provide a bar graph representation of the number of projects per "
                       f"{SpatialFeatureChoice_Text_Single.lower()} with the colour of the bars representing the level of"
                       f" capital demand in each of these {SpatialFeatureChoice_Text_Single.lower()}s."}
            # *********************************************************************************************************
            ...
            if chosen_feature_qty > 10:  # If there are enough features to show 5 highest
                cells = table.add_row().cells
                cells[
                    0].text = f"The 5 {SpatialFeatureChoice_Text_Single.lower()}s that collectively have with the highest number" \
                              f" of projects"
            ...
            if chosen_feature_qty > 10:
                cells = table.add_row().cells
                cells[
                    0].text = f"The 5 {SpatialFeatureChoice_Text_Single.lower()}s that collectively have collective the highest " \
                              f"capital demand:"
            ...
            headings3 = {"1": f"Total Capital Demand per {SpatialFeatureChoice_Text_Single.lower()} - All Years"}
            ...
            paragraphs9 = {
                "1 n": f"The following figures provide a histogram representation of the capital demand per"
                       f" {SpatialFeatureChoice_Text_Single.lower()}. A colour breakdown is provided of each year's sub-total"
                       f" that contribute towards the overall total capital demand of each "
                       f"{SpatialFeatureChoice_Text_Single.lower()}. Note that the capital demand beyond the first three years"
            ...
            paragraphs11 = {"1 n": f"\nThe following figures provide a histogram representation of the capital demand"
                                   f" per {SpatialFeatureChoice_Text_Single.lower()}. A colour breakdown is provided of each "
                                   f"year's sub-total that contribute towards the overall MTREF total capital demand "
                                   f"of each {SpatialFeatureChoice_Text_Single.lower()}."}
            # *********************************************************************************************************
            ...
            cells[
                2].text = f"The average MTREF capital demand per {SpatialFeatureChoice_Text_Single.lower()}:" \
                          f"\n{average_cost_MTREF}"
            ...
            for feature in list_of_features:
                ...
                paragraphs17 = {"1 n": f"\nProjects that do not have any recorded budget demand on the system at the "
                                       f"time that this report was requested, are shown in the table with zero budgets "
                                       f"indicated."}
                # ****************************************************************************************************
                ...
                for project in list(df_perward[feature].index):
                    ...
                    try:
                        ...
                        cells = table.add_row().cells
                        cells[0].text = f"{project}"
                        project_name = df_ProjectCatalogue[df_ProjectCatalogue['ProjectId'] == project]['Name'].iloc[0]
                        cells[1].text = f"{project_name}"
                        department_name = df_ProjectCatalogue[df_ProjectCatalogue['ProjectId'] == project]['DepartmentName'].iloc[0]
                        cells[2].text = f"{department_name}"
                        cells[3].text = f"R0.00 (0%)\nNo budget request."
                # ****************************************************************************************************
                #                         End Table 5.x2 All Projects in Spatial Feature                             #
                # ****************************************************************************************************
                par_formatter(paragraphs16)
                par_formatter(paragraphs17)
                # ****************************************************************************************************
