...
def home():
    global show_drop_down, all_well, url_choice, url_list, spatial_var, username, password, grant_type
    ...
    if json_file_ok:
        if request.method == 'POST' and not show_drop_down:  # Step 1: Pressed the submit button with username and pw, no dropdown yet
        ...
        elif request.method == 'POST' and show_drop_down: # Step 3: Pressed the submit button with spatial features selected
            SpatialFeatureChoice = request.form['inputGroupSelect01']

            # Call the MapServiceIntersectionCatalogue - it can only be called now that the preferred spatial feauture
            # is selected by the user (SpatialFeatureChoice).
            ...
            if df_MapServiceIntersections.empty:
                ...
                return render_template('home.html', show_drop_down=show_drop_down, url_choice=url_choice,
                                       url_list=url_list, spatial_var=spatial_var)
            else: # All APIs have now been called so the report building can proceed
                ...
                show_drop_down = False  # To allow a new choice to be made
        ...
        else:  # Step 2: All the API's were called successfully, so show the DropDown of features to choose from
